...
var kayitbaglanti=encodeURIComponent($('#kayitbaglanti').val());
//alert(encodeURIComponent($('#kayitbaglanti').serialize()));
//alert(decodeURIComponent($('#kayitbaglanti').serialize()))
...
if (valid!='') {	
    ...
    var kayitformdatastr ='kayitad=' + kayitad + '&kayitdetay='+ kayitdetay + '&sosyalbaglanti='+ kayitbaglanti +'&anahtarlar=' + anaht;
    ...
    islem  ='1';
    kayitformdatastr = kayitformdatastr.replace(/[!'()]/g, escape).replace(/\*/g, "%2A") + '&kayitid=' + kayitid;
    }
...
function kayitjqislem(kayitformdatastr,islem){
	if (islem=='0'){var islemurl='../islemler/islemler.php?islem=sosyalkayitekle'}
	else{var islemurl='../islemler/islemler.php?islem=sosyalkayitduzenle'}
	$.ajax({	
		type: "POST",
		url: islemurl,
...
<tr>
    <td width='20%' valign="top">Sosyal Bağlantı:</td><td><?=$form->doInput(array('kayitbaglanti',str_replace("kayitbaglanti=","",urldecode($ddata->kayitbaglanti))));?></td>
</tr>
<tr>
    <?php
